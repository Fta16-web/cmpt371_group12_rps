...
def outcome_handler(choice):
    ...
    client_choice_var.set("Player 1 choice: " + choice.capitalize())
    host_choice_var.set("Player 2 choice: " + host_choice.capitalize())
...
def update_scores():
    label_client_score.config(text="Player 1 score: " + str(Player1_score))
    label_host_score.config(text="Player 2 score: " + str(Player2_score))

    if Player1_score >= 3 or Player2_score >= 3:
        winner = "Player 1" if Player1_score >= 3 else "Player 2"
...
def reset_choices():
    client_choice_var.set("Player 1 choice: ")
    host_choice_var.set("Player 2 choice: ")
    outcome_var.set("")
...
def wait_for_player2():
    ...
    if msg == "Player 2 connected. Game starting!":
        message_label.config(text="Player 2 connected. Game starting!")
        rock_label.config(state=tk.NORMAL)
        paper_label.config(state=tk.NORMAL)
        scissors_label.config(state=tk.NORMAL)
master = tk.Tk()
master.title("RSP")
...
label_client_score = tk.Label(master, text="Player 1 score: 0", font=("calibri", 16), fg="white", bg="black")
label_client_score.grid(row=1, column=0, sticky="w", padx=20)

label_host_score = tk.Label(master, text="Player 2 score: 0", font=("calibri", 16), fg="white", bg="black")
...
message_label = tk.Label(master, text="Waiting for Player 2 to connect...", font=("calibri", 16), fg="white", bg="black")
message_label.grid(row=5, column=0, columnspan=3, pady=20)
